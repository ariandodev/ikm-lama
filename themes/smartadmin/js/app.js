...
function notifikasi2(title, msg, type, isrefresh) {
	...
	$.smallBox({
		title : title,
			...
			timeout : 30000 // lama alert ditampilkan
		});
}
...
function notifikasi(title, msg, type, istimeout, isrefresh) {
	...
	window.THEMES_URI = 'themes/smartadmin/';

	/*loadScript(window.THEMES_URI+"js/plugin/jquery-priceformat/jquery.price_format.2.0.min.js", function(){
		setPriceFormat();
	});*/
	...
	function AppLock() {
		if (window.isLogin) {
			$.get('site/logout', function(data) {/*lets logout*/});
